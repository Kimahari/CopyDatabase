...
namespace DataBaseCompare.Models {
    ...
    public class ScriptedModel : ModelBase {
        ...
        internal virtual async Task CopyTo(ConnectionModel sourceModel, ConnectionModel destinationModel, String databaseName, bool copyData, CancellationToken token, SqlConnection connection, SqlTransaction transaction, Action<long> callback = null) {
    ...
    public class TableModel : ScriptedModel {
        ...
        internal override async Task CopyTo(ConnectionModel sourceModel, ConnectionModel destinationModel, String databaseName, bool copyData, CancellationToken token, SqlConnection connection, SqlTransaction transaction, Action<long> callback = null) {
            string sql = await ScriptTable(sourceModel, databaseName);
            this.CurrentReportCallback = callback;
            //destinationConnection.BuildConnection(name)
            await Task.Factory.StartNew(() => {
        ...
        private void onRowsCopied(object sender, SqlRowsCopiedEventArgs e) {
            this.CurrentReportCallback?.Invoke(e.RowsCopied);
        }

        private Task<string> ScriptTable(ConnectionModel sourceModel, string databaseName) {
...
SELECT @SQL";
                    ...
                    retval = connection
                        ...
                        ?.Replace("IDENTITY(*,1)", $"IDENTITY(1,1)");
                }
